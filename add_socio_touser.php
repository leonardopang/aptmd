...
function field_user()
{
    ...
    if(isset($_GET['addsocio'])):
        $socio = $_GET['addsocio'];
        $max_socio = intval($wpdb->get_var("SELECT MAX(CAST(meta_value as UNSIGNED)) FROM $wpdb->usermeta WHERE meta_key = 'Socio'")) + 1;
        update_user_meta($socio, "Socio", $max_socio);
    endif;
    ...
    <table>
        ...
        <tbody>
            <?php
            global $wpdb;
            $users = $wpdb->get_results("SELECT u.ID, u.user_login, u.user_email, m2.meta_value as 'Tipo', m3.meta_value as 'Pago' 
            FROM wp_users u 
            LEFT JOIN wp_usermeta m2 ON u.ID = m2.user_id AND m2.meta_key = 'Socio Type' 

...
function canalizacaoadm()
{
    ...
    if (isset($_GET['delete'])) {
        $delete = $_GET['delete'];
        $aluno = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "aptmd_formador_formados WHERE `key` = '{$delete}'")[0];
        $formador_edit = $aluno->id_formador;
        update_user_meta($formador_edit, 'canalizacao', intval(get_user_meta($formador_edit, 'canalizacao', true)) + 1);
        $wpdb->delete($wpdb->prefix . 'aptmd_formador_formados', array('key' => $delete));
    ...
    if (!empty($search_term)) {
        $alunos_formados = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "aptmd_formador_formados WHERE nome_aluno LIKE '%$search_term%' OR email_aluno LIKE '%$search_term%' or local LIKE '%$search_term%'");
    } else {
        $alunos_formados = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "aptmd_formador_formados");
    }
    ...
    <table class="meus_alunos">
        <thead class="table_header">
            <tr class="header_row">
                ...
                <th class="header_elem">Formador 2</th>
        ...
        <tbody class="table_body">
            <?php foreach ($alunos_formados as $aluno) :
                ...
                $formador2 = $aluno->id_formador2 == 0 ? null : get_user_by('id', $aluno->id_formador2);
            ?>
                <tr class="body_row">
                    <td class="body_elem"><?php echo $formador->first_name; ?></td>
                    <td class="body_elem"><?php echo ($formador2 != null) ? $formador2->first_name : ""; ?></td>
                    ...
                    <td class="body_elem">
                        <a class="opcao" href="<?php echo add_query_arg(array("delete" => $aluno->key)) ?>">Apagar Certificado e Devolver Crédito</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
add_shortcode('canalizacaoadm', 'canalizacaoadm');
?>
...
function certificados()
{
            ...
            if ($certi < count($name)) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    try {
                        ...
                        $imagick->readImage("/home/aptmd.org/public_html/Certificado TMD - " . $name[$i] . ".svg");

                        $imagick->setImageFormat('pdf');

                        $imagick->writeImage("/home/aptmd.org/public_html/Certificado TMD - " . $name[$i] . ".pdf");
                    } catch (Exception $e) {
    ...
    <?php else: ?>
        ...
        <?php endif; ?>
        <?php if ($error == -1): ?>
            <h3 class="error">A carga horria deve ser maior que 12H</h3>
        <?php endif; ?>
        <?php if ($error <= 0): ?>
            <div class="modal">
                            ...
                            alunos.forEach(aluno => {
                                ...
                                email.addEventListener('change', (e) => {
                                    ...
                                    if (inputEmail == userEmail) {
                                        ...
                                        errorMessage.innerHTML = "O e-mail não pode ser igual ao teu e não será gerado!";
    ...
    <script>
        button.addEventListener('click', (e) => {
            ...
            aluno.innerHTML = `
                                            <p class="titulo">Formando</p>
                                            <label for="english[]">Marca a caixa para emitir este Certificado em Ingls:</label>
                                            <input type="checkbox" name="english[]" class="english">
                                            <label for="aluno_name[]">Nome Formando:</label>
                                            <input type="text" name="aluno_name[]" required>
                                            <label for="aluno_email[]">Email Formando:</label>

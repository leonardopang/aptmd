...
function validar_socio()
{
    ...
    if ($type == '' && $socion == '') {
?>
    <h1>Querida alma, a tua ficha de sócio ainda não foi validada.</h1>
<?php return; } ?>

<h1 class='TAG'>Quando realizar o pagamento da tua quota retorna a esta página para verificar as tuas informações!</h1>
    <form class="form_valid" action="" method="post">
        <div class="Socio_container">
            <h1 class='TAG'>As Tuas Informações</h1>
            <h2 class="TAG">Nome: <h3 class="info"> <?php echo $display_name ?></h3>
            </h2>
            <?php if ($valid == '1') : ?>
                <h2 class="TAG">Tipo de Sócio: <h3 class="info"> <?php echo $meta['Socio Type'][0] ?></h3>
                </h2>
            <?php endif;
            if($valid == '') : ?>
                <h2 class="TAG">Tipo de Sócio:
            ...
            <?php endif; ?>
            <?php if ($valid == '1') : ?>
                <h1 class="parabens">Querida Alma parabéns, és sócia da APTMD.</h1>
            <?php else : ?>

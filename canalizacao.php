...
function canalizacao()
{
    ...
    if (
        ...
        if (intval($_POST['carga_horaria']) < 12) {
            ...
            $nascimento = $_POST['data_aniversario'];
            ...
            if (count($name) > $certi) {
                ...
                if (isset($_FILES['assinatura']) && $_FILES['assinatura']['error'] == 0) {
                    ...
                    // var_dump(file_get_contents($assinatura['tmp_name']));
                ...
                for ($i = 0; $i < $counts; $i++) {
                    ...
                    // $checagem = $wpdb->get_results("SELECT * FROM wp_aptmd_formador_formados WHERE `key` = '" . $certificado->get_key() . "'");
                    // if ($checagem) {
                    //     $error = -3;
                    //     $emitidos[] = $name[$i];
                    //     continue;
                    ...
                    $wpdb->insert(
                        $wpdb->prefix . 'aptmd_formador_formados',
                        $cert_data
                    );
                    ...
                    try {
                        ...
                        $imagick = new Imagick();
                        $imagick->readImage("/home/aptmd.org/public_html/Certificado Canalização - " . $name[$i] . ".svg");
                        $imagick->setImageFormat('pdf');
                        $imagick->writeImage("/home/aptmd.org/public_html/Certificado Canalização - " . $name[$i] . ".pdf");
                    ...
                    Se no colocaste a tua assinatura digital no formulário, assina o certificado antes da entrega.<br><br>
                    Cumprimentos de Luz,<br>
                    Equipe APTMD<br>
                    Atenciosamente";
                        $message2 = "Olá, " . $name[$i] . ",<br><br>
                    Segue em anexo teu certificado da tua formação que participou no Workshop de Canalizao de " . $data_inicio . " a " . $data_fim . "<br><br><br><br>
                    Cumprimentos de Luz,<br>
                    Equipe APTMD<br>
                    Atenciosamente";
                        ...
                        unlink(ABSPATH . "Certificado Canalização - " . $name[$i] . ".svg");
                        unlink(ABSPATH . "Certificado Canalização - " . $name[$i] . ".pdf");

                    } catch (Exception $e) {
                        echo "Erro: Contate o suporte -> ";
                        var_dump($e);
                    }
                }
            }
    ...
    <?php else: ?>
        <?php if ($error == -3): ?>
            <h3 class="error">Os seguintes certificados já foram emitidos:
        ...
        <?php if ($error == -2): ?>
            <h3 class="error">A Data de Início tem que ser menor ou Igual a Data de Fim!</h3>
        <?php endif; ?>
        ...
        <?php if ($error <= 0): ?>
            ...
            <h1 class="certificadosh1">Saldo:
                <?php echo $certi ?> certificados
            </h1>
            <form class="formfile" action="" method="post" enctype="multipart/form-data">
                <p class="assinatudasCheckLabel" for="csv_file">Se quiseres pré-carregar multiplos alunos simultaneamente, faz o
                    upload de um ficheiro .csv com NOME, EMAIL, DATA DE ANIVERSARIO no formato YYYY-MM-DD<br></p>
            ...
            <form class="certificados" method='post' enctype="multipart/form-data">
                <div class='flex'>
                    <div class="flex_start">
                        ...
                        <p class='assinatudasCheckLabel'>*Marca a caixa para emitir e enviar o(s) certificado(s) assinados
                            digitalmente <span style="text-decoration:underline;">automaticamente</span>:<br></p>
                    </div>
                    <p class="SUBassinatudasCheckLabel">*Para emitir o (s) certificado (s) para impressão avança sem marcar a caixa
                        ou cria novamente um novo conjunto de certificado (s).</p>
                </div>
                <div class="assinaturasDIV" hidden>
                    <label for="assinatura"><strong>(Opcional)</strong> Para emitir certificados já assinados, carrega uma imagem
                        com a tua assinatura com estes requisitos:<br>
                    ...
                    <?php if ($formador != 'null'): ?>
                        <label for="assinaturad"><strong>(Opcional)</strong> Carregue uma imagem dentro dos mesmos requisitos
                            estabelecidos contendo a assinatura do outro formador para emitir os certificados já assinados.
                        </label>
                        <input type="file" name="assinaturad" class="assinatura" accept="image/*">
                    <?php endif; ?>
                    <h1 class="subtitle" style="font-size: small;">Se enviares a imagem da tua assinatura, o teu aluno recebe o
                        certificado diretamente por email.</h1><br>
                </div>
                <script>
                ...
                <div class="container">
                    <div class="localizacao">
                        <p class="titulo">Localizaão</p>
                        <label for="cidade">Cidade:</label>
                        <input type="text" name="cidade" id="cidade" required>
                        <label for="pais">Pais:</label>
                        <input type="text" name="pais" id="pais" required>
                        <label for="espacoformacao">Espaço de Formaço:</label>
        ...
        <?php elseif ($error == 1): ?>
            <h1 class="certificadosh1">Teu Saldo:
                <?php echo $certi ?> certificados
            </h1>
            <h5 class="certificadosh2">Tua quantidade de certificados é insuficiente para avançar a solicitação!</h5>
            <h5 class="certificadosh2">Tens uma encomenda? Por favor, aguarde o processamento ou entre em contacto com: <a
                    href="mailto:certificados@aptmd.org">certificados@aptmd.org</a>
            </h5>
        <?php endif; ?>
    <?php endif;
    ?>
        ...
        button.addEventListener('click', (e) => {
            ...
            email.addEventListener('change', (e) => {
                ...
                if (inputEmail == userEmail) {
                    ...
                    errorMessage.innerHTML = "Se o email for igual ao teu, o certificado não será emitido.";
                    errorMessage.style.color = "red";
                    errorMessage.style.fontSvaize = "14px";
                    email.after(errorMessage);
                } else {
                    const errorMessage = document.querySelector(".errorEmail");
                    if (errorMessage) {
